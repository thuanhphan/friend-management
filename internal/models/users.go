...
type userR struct {
	FriendRelationships RelationshipSlice `boil:"FriendRelationships" json:"FriendRelationships" toml:"FriendRelationships" yaml:"FriendRelationships"`
...
func (r *userR) GetFriendRelationships() RelationshipSlice {
	if r == nil {
		return nil
	}
	return r.FriendRelationships
}

func (r *userR) GetRelationships() RelationshipSlice {
	if r == nil {
		return nil
	}
...
// FriendRelationships retrieves all the relationship's Relationships with an executor via friend_id column.
func (o *User) FriendRelationships(mods ...qm.QueryMod) relationshipQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"relationships\".\"friend_id\"=?", o.ID),
	)
...
func (o *User) Relationships(mods ...qm.QueryMod) relationshipQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"relationships\".\"user_id\"=?", o.ID),
	)

	return Relationships(queryMods...)
}

// LoadFriendRelationships allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadFriendRelationships(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`relationships.friend_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.FriendRelationships = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.FriendID {
				local.R.FriendRelationships = append(local.R.FriendRelationships, foreign)
				if foreign.R == nil {
					foreign.R = &relationshipR{}
				}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadRelationships(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`relationships`),
		qm.WhereIn(`relationships.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// AddFriendRelationships adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.FriendRelationships.
// Sets related.R.Friend appropriately.
func (o *User) AddFriendRelationships(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Relationship) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			FriendRelationships: related,
		}
	} else {
		o.R.FriendRelationships = append(o.R.FriendRelationships, related...)
	}
...
func (o *User) AddRelationships(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Relationship) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
