...
func testUserToManyFriendRelationships(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.FriendRelationships().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadFriendRelationships(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.FriendRelationships); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.FriendRelationships = nil
	if err = a.L.LoadFriendRelationships(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.FriendRelationships); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyRelationships(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.Relationships().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadRelationships(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Relationships); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Relationships = nil
	if err = a.L.LoadRelationships(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Relationships); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpFriendRelationships(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddFriendRelationships(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.FriendRelationships[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.FriendRelationships[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.FriendRelationships().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpRelationships(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddRelationships(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Relationships[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Relationships[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Relationships().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `Email`: `character varying`}
	_           = bytes.MinRead
)
